...
import romanow.abc.ess2.android.lep500.FFTCallBackPlus;
import romanow.abc.ess2.android.lep500.ListBoxDialog;
...
public abstract class BaseActivity extends AppCompatActivity implements I_Notify {
    ...
    public void paintOne(double freq) {
        paintOne(multiGraph, freq,deffered.get(0).getNormalized(), getPaintColor(0), 0, 0, true);
        }
    ...
    public void defferedFinish(){
        ...
        for(int i=0;i<deffered.size();i++){
            FFTStatistic statistic = deffered.get(i);
            int color = getPaintColor(i);
            addToLog(false,String.format(defferedList.get(i).toString()),greatTextSize,color);
            addToLog(false,"Уровни (волна): "+statistic.getWave().deciBellToString(false),middleTextSize,color);
            //---------------------- Уровни спектра ??????? ------------------------------------------------
            //addToLog(false,"Уровни (спектр): "+statistic.getSumT().deciBellToString(true),middleTextSize,color);
            showStatisticFull(statistic,i);
            paintOne(multiGraph,defferedList.get(i).getFileFreq(),statistic.getNormalized(),getPaintColor(i),0,0,true);
            }
        }
    ...
    public void normalize(){
        ...
        for(FFTStatistic statistic : deffered) {
            double max2 = statistic.normalizeStart(ctx.settings.nTrendPointsSpectrum);
            if (max2 > max)
                max = max2;
            }
        for(FFTStatistic statistic : deffered) {
    ...
    public void processInputStream(FileDescription fd, InputStream is, String title) throws Throwable{
        processInputStream(true,fd,is,title,new FFTAdapter(this,title));
        }
    ...
    public void processInputStream(boolean toLog,FileDescription fd, InputStream is, String title, FFTCallBackPlus adapter) throws Throwable{
        ...
        FFTParams params = new FFTParams().W(ctx.settings.p_BlockSize* FFT.Size0).procOver(ctx.settings.p_OverProc).
                compressMode(false).winMode(ctx.settings.winFun).freqHZ(fd.getFileFreq());
        FFT fft = new FFT();
