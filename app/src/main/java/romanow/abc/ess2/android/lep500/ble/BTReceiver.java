...
public class BTReceiver{
    ...
    public synchronized void startMeasure(BTTextFile file0, boolean tested) {
        startMeasure(file0,-1,tested);
        }
    public synchronized void getChargeLevel()  {
        sendCommand(SENSOR_CMD_CHARGE_LEVEL,0);
        }
    public synchronized void startMeasure(BTTextFile file0, int duration, boolean tested) {
        if (isWorking()){
